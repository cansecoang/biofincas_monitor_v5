...
function ProductsLayoutContent({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const fetchCatalogs = async () => {
      try {
        ...
        if (organizationsData.organizations) setOrganizations(organizationsData.organizations);
      } catch (error) {
  ...
  const updateURL = (outputId: string, organizationId: string, productId: string) => {
    const params = new URLSearchParams();
  ...
  const handleProductChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    updateURL(selectedOutput, selectedOrganization, value);
  };
    ...
    try {
      ...
      if (result.success) {
        ...
        if (refreshData.success) {
          ...
          setSelectedProduct('');
          updateURL(selectedOutput, selectedOrganization, '');
        }
          ...
          <p className="text-gray-600 text-base">
            {pathname === '/products/matrix' ? (
              ...
              <span className="flex items-center gap-1">
                <span className="font-bold">Organization:</span> {selectedProductData.organization_name || 'N/A'} • 
