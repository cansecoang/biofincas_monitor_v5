...
export async function GET(request: NextRequest) {
  try {
    ...
    // Si se proporciona taskId, retornar solo esa tarea
    if (taskId) {
      const taskQuery = `
        SELECT
          t.task_id               AS id,
    ...
    if (!productId) {
      return NextResponse.json({ success: false, error: 'Product ID or Task ID is required' }, { status: 400 });
    }

    const offset = (page - 1) * limit;
    
    // Mapeo de columnas del frontend a las columnas reales de la BD (whitelist para prevenir SQL injection)
    ...
    const query = `
      ...
      ORDER BY ${orderByColumn} ${validSortOrder} NULLS LAST, t.task_id
